use iced::{
	button, scrollable, slider, text_input, Align, Button, Checkbox, Column, Container, Element, Length, ProgressBar, Radio,
	Row, Rule, Sandbox, Scrollable, Settings, Slider, Space, Text, TextInput,
};
...
impl Sandbox for Styling {
	...
	fn view(&mut self) -> Element<Message> {
		let choose_theme = style::Theme::ALL.iter().fold(
			...
			|column, theme| {
				column.push(
					Radio::new(*theme, &format!("{:?}", theme), Some(self.theme), Message::ThemeChanged).style(self.theme),
				)
			},
		);

		let text_input = TextInput::new(&mut self.input, "Type something...", &self.input_value, Message::InputChanged)
			.padding(10)
			.size(20)
			.style(self.theme);
		...
		let slider = Slider::new(&mut self.slider, 0.0..=100.0, self.slider_value, Message::SliderChanged).style(self.theme);
